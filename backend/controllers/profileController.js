...
export const getMyProfile = async (req, res) => {
  try {
    ...
    const skills = await pool.query(
      `SELECT us.id, us.proficiency_level, us.years_experience,
              us.is_verified, us.verification_source,
              s.id as skill_id, s.name, s.category
       FROM user_skills us
       JOIN skills s ON s.id = us.skill_id
       WHERE us.user_id = $1 ORDER BY s.category, s.name`,
      [user.id]
    );
...
export const getPublicProfile = async (req, res) => {
  try {
    ...
    const skills = await pool.query(
      `SELECT us.proficiency_level, us.is_verified, s.name, s.category
...
export const addSkill = async (req, res) => {
  try {
    ...
    const skill = await pool.query('SELECT name FROM skills WHERE id = $1', [skill_id]);
    ...
    const result = await pool.query(
      `INSERT INTO user_skills (user_id, skill_id, proficiency_level, years_experience)
       ...
       ON CONFLICT (user_id, skill_id) DO UPDATE
         SET proficiency_level = $3, years_experience = $4
       RETURNING *`,
      [req.user.id, skill_id, proficiency_level, years_experience || null]
    );
